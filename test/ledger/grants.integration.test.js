...
test('get cohort', async (t) => {
  t.throws(getCohort, Error, 'requires 2 arrays')
  ...
  t.is('grant', getCohort([{ type: 'ugp', grantId }], [grantId]), 'if type is ugp, use grant')
  t.is('grant', getCohort([{ grantId }], [grantId]), 'if type is empty, use grant')
  t.is('ads', getCohort([{ type: 'ads', grantId }], [grantId]), 'if type is ads, use ads')
  t.is('android', getCohort([{ type: 'android', grantId }], [grantId]), 'if type is android, use android')

  const aGrantId = uuidV4()
  const bGrantId = uuidV4()
  const cGrantId = uuidV4()
  const aGrant = { type: 'a', grantId: aGrantId }
  const bGrant = { type: 'b', grantId: bGrantId }
  const cGrant = { type: 'c', grantId: cGrantId }
  const abcGrants = [aGrant, bGrant, cGrant]
  t.is('b', getCohort(abcGrants, [uuidV4(), bGrantId]), 'finds matching grant out of many')
})
