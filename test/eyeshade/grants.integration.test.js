...
test('grant type is constriained', async (t) => {
  const client = await postgres.connect()
  const grant = createGrant()
  const noTypeGrant = Object.assign({}, grant, { id: uuidV4(), type: 'not-a-type' })
  await t.throwsAsync(() => insertGrant({ postgres, client, grant: noTypeGrant }))
  const autoContGrant = Object.assign({}, grant, { id: uuidV4(), type: 'auto-contribute' })
  await insertGrant({ postgres, client, grant: autoContGrant })
  const oneoffGrant = Object.assign({}, grant, { id: uuidV4(), type: 'oneoff-tip' })
  await insertGrant({ postgres, client, grant: oneoffGrant })
  const recurringGrant = Object.assign({}, grant, { id: uuidV4(), type: 'recurring-tip' })
