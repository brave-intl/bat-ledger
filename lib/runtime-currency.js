...
const monitor = (config, runtime) => {
  ...
  client.on('coreHub', 'updateSummaryState', async (data) => {
    ...
    try { tickers = await inkblot(config, runtime) } catch (ex) {
      ...
      now = underscore.now()
      if (singleton.stamp > now) return

      singleton.stamp = now + (15 * msecs.minute)
...
// when altcurrency is bitcoin, then probi is denominated in satoshis
