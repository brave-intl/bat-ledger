...
create table balance_snapshots(
  id         uuid primary key,
  total      numeric(28, 18) default 0.0 check (amount > 0.0),
...
create table balance_snapshot_accounts(
  ...
  balance      numeric(28, 18) not null check (amount > 0.0)
);

create index on balance_snapshot_rows(snapshot_id, account_id, account_type);
