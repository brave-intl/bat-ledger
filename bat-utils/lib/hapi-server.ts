...
import rateLimiter from 'hapi-rate-limiter'
 ...
 */
            ...
            if ((graylist.authorizedAddrs.length) &&
      ...
      if (process.env.NODE_ENV === 'production') {
        await new Promise((resolve, reject) => {
          server.register({ register: hapiRequireHttps, options: { proxy: true } }, (err) => {
            if (err) {
  ...
  server.on('log', (event, tags) => {
    ...
    logger.forEach((entry) => {
      if ((entry.data) && (typeof entry.data.msec === 'number')) {
        (params.request as any).duration = entry.data.msec
      }
    })
