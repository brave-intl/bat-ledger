...
Currency.knownRateKeys = knownRateKeys
Currency.decimals = decimals

Currency.Cache = Cache

export default Currency
Currency.global = generateGlobal
...
Currency.prototype = {
  ...
  alt2fiat: async function (altcurrency, probi, currency, floatP) {
    let rate = await this.ratio(altcurrency, currency)
    if (!rate) {
      return
    }
    const entry = currencyCodes.code(currency)
    const scale = this.alt2scale(altcurrency)
    let amount
...
function generateGlobal (config, runtime): void {
  if (!singleton) {
...
function Currency (config, runtime?): void {
  const context = this
